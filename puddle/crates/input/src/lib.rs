...
use window::winit::{event::{DeviceEvent, RawKeyEvent, WindowEvent}, keyboard::PhysicalKey};
...
impl Plugin for InputPlugin {
    fn finish(&mut self, app: &mut Application) {
        ...
        app.scheddules.add_non_parralel(application::Scheddules::Startup, event_listen);
...
fn event_listen(
    ...
    use window::winit::event::{Event, WindowEvent, DeviceEvent};

    window_events.handler.connect(move |event| match event {
        Event::DeviceEvent { device_id:_, event} => match event {
            DeviceEvent::MouseMotion { delta } => {
                input.lock().unwrap().event_handler.fire(&mut InputEvents::MouseDelta(delta.0, delta.1));
            }
            DeviceEvent::Key(RawKeyEvent {
                ...
                if let PhysicalKey::Code(code) = physical_key {
                    ...
                    input.event_handler.fire(&mut InputEvents::KeyPressed(*code, state.is_pressed()));
        ...
        Event::WindowEvent { window_id:_, event } => match event {
            WindowEvent::CursorMoved { device_id:_, position } => {
                input.lock().unwrap().event_handler.fire(&mut InputEvents::MouseMoved(*position));
            }
            _=> {}
        }
