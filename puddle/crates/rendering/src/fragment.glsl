...
void main() {
    vec2 uv = (gl_FragCoord.xy / uniforms.screen_size.xy);
    uv = uv * 2.0 - 1.0;
    uv.x *= uniforms.screen_size.x / uniforms.screen_size.y;
    uv.y -= uv.x * uv.x * 0.15;
    uv.y *= -1.0;
    vec3 camPos = vec3(3., -1.5, uniforms.time * 5.);
    ...
    f_color = vec4(1. - col.xyz, clamp(1. - col.w / MAXDIST, 0., 1.));
}
