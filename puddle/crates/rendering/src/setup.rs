...
pub struct ExampleSchene {
    ...
    pub vertex_buffer: Subbuffer<[Vertex3]>,
}

pub fn init(app: &mut application::Application) {

    let window = &app.resources.get::<window::PuddleWindow>().unwrap().window.clone();
    ...
    mod vs {
        vulkano_shaders::shader! {
            ...
            src: r"
                ...
                void main() {
                    gl_Position = vec4(position.yzx, 1.0);
                }
            ",
        }
    }
    ...
    let pipeline = {
        ...
        let vertex_input_state = Vertex3::per_vertex().definition(&vs).unwrap();
