...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  ...
  let cam_pos = (camera.camera_eye.xyz - model_position) * model_rotation;
  let dir = normalize(in.uv_cords - cam_pos);
...
fn RayCast(campos: vec3<f32>, dir: vec3<f32>) -> vec4<f32> {
  ...
  let cubeMin = vec3(-0.5 * chunk_res);
  let cubeMax = vec3(0.5 * chunk_res);
  ...
  while current_dis < max_dis {
    ...
    if MapCheckX > cubeMax.x || MapCheckY > cubeMax.y || MapCheckZ > cubeMax.y || MapCheckX < cubeMin.x || MapCheckY < cubeMin.x || MapCheckZ < cubeMin.x {
      break;
    }
  }
